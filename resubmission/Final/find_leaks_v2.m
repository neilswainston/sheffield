function [list_of_leaks_v2_blocking, causing_impossible_growth_when_max, causing_impossible_growth_when_min] = find_leaks_v2(model)
...
model_t.ub(findRxnIDs(model_t, biomassID))                  = 0;
model_t.lb(findRxnIDs(model_t, biomassID))                  = 0;
model_t.ub(findRxnIDs(model_t, mAbID))                      = 0;
model_t.lb(findRxnIDs(model_t, mAbID))                      = 0;
    ...
    for i=1:rxns_counter1
        if ~(ismember(i,indexExhange)) %excludes knockedout exchange rxns
            if ((sol_bio.x(i) > zero_flux) || (sol_bio.x(i) < -1*zero_flux))
                list_of_leaks_v2_blocking          = [list_of_leaks_v2_blocking; model_t.rxns(i), sol_bio.x(i)];
            end
        end
    end
end
...
for i=1:rxns_counter1
    if ~(ismember(i,indexExhange))
        ...
        if ((sol_bio3.x(biomass_index) > zero_flux) || (sol_bio3.x(biomass_index) < -1*zero_flux))
            causing_impossible_growth_when_max = [causing_impossible_growth_when_max; model_t.rxns(i), model_t.rxnsNames(i), sol_bio3.x(i), 'biomass flux:', sol_bio3.x(biomass_index)];
        end
    end
end
...
for i=1:rxns_counter1
    if ~(ismember(i,indexExhange))
        ...
        if ((sol_bio4.x(biomass_index) > zero_flux) || (sol_bio4.x(biomass_index) < -1*zero_flux))
            causing_impossible_growth_when_min = [causing_impossible_growth_when_min; model_t.rxns(i), model_t.rxnsNames(i), sol_bio4.x(i), 'biomass flux:', sol_bio4.x(biomass_index)];
        end
...
% % model_t.ub([findRxnIDs(model_t, rxnIDsExchange)])       = 0;
% 
% [sol_con_obj1, sol_con_obj2, sol_con_both] = constrainOnlyOne_and_test_for_sol_equality(model_t, reactionID, con_level, biomassID, mAb_index);
% 
